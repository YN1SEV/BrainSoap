...
async function updateStatus() {
  ...
  if (!timer) { minEl.textContent = 'no limit'; bar.style.setProperty('--time-bar-progress', '0%'); return; }
  minEl.textContent = `${timer.remaining} min`;
  const usedPct = timer.maxTime > 0 ? Math.round(((timer.maxTime - timer.remaining) / timer.maxTime) * 100) : 0;
...
const setFocusEnabled = (enabled) => { focusToggle().disabled = !enabled; };
...
async function onFocusChange(e) {
  if (e.target.checked) {
    ...
    if (!confirm('Turn off Focus Mode?')) { e.target.checked = true; return; }
    await turnFocusOff();
  }
  await updateStatus();
}
...
async function onPauseChange(e) {
  if (e.target.checked) {
    if (!confirm('Pause BrainSoap? This disables all tracking and blocking.')) { e.target.checked = false; return; }
    await custom_storage.setLocal('paused', true);
    if (focusToggle().checked) { focusToggle().checked = false; await turnFocusOff(); }
